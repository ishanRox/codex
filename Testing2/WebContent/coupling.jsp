...
<div class="size1">
    <div id="pageintro" class="hoc clear"> 
			...
			<div>
				<h2>Coupling</h2><br>
					<p>Ccp = Nr + Nmcms + Nmcmd + Nmcrms + Nmcrmd + Nrmcrms + Nrmcrmd + Nrmcms + Nrmcmd + Nmrgvs + Nmgvd + Nrmrgvs + Nrmrgvd</p>
			</div>
			<footer>
				<div class="dropdown">
...
<form action="" method="post"
		...
		<tr>
			...
			<td> <input value="2" type="number" name="0" /></td>
		</tr>
		
		<tr>
			<td>A regular method calling another regular method in the same file</td>
			<td> <input value="2" type="number" name="1" /></td>
		</tr>
		<tr>
	...
	String contentType = request.getContentType();
	 if ((contentType!=null)&&(contentType.indexOf("multipart/form-data") >= 0)) {
		...
		try {
			...
			while (i.hasNext()) {
			    ...
			    if (item.isFormField()) {
			      ...
			      System.out.println(name+" fffffffffffffffffffffffffffff "+value);
		...
		if (!fi.isFormField()) {
			...
			System.out.println(fileName+" dddddddddddddddddddd ");
	...
	<table style="width: 100%">
		...
		<tbody>
			...
			<tr>
				...
				<%
					table5.put(number,((isRecursiveMethod[0]) ? defaultWeight.get(0) : 0) + normalToNormalVal[0] * defaultWeight.get(1)
							+ normalToRecursiveVal[0] * defaultWeight.get(3) 
							+ RecursiveToRecursiveVal[0] * defaultWeight.get(5) 
							+ RecursiveToNormalVal[0] *  defaultWeight.get(7)  + globalUsedByNonR *  defaultWeight.get(9) 
							+ globalUsedByR * defaultWeight.get(11));
				%>
				<td><%=((isRecursiveMethod[0]) ? defaultWeight.get(0) : 0) + normalToNormalVal[0] * defaultWeight.get(1)
						+ normalToRecursiveVal[0] * defaultWeight.get(3) 
						+ RecursiveToRecursiveVal[0] * defaultWeight.get(5) 
						+ RecursiveToNormalVal[0] *  defaultWeight.get(7)  + globalUsedByNonR *  defaultWeight.get(9) 
						+ globalUsedByR * defaultWeight.get(11) %></td>
